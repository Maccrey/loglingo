...
- [x] **지원 언어 15개국 리소스 추가**:
    - 한국어, 영어, 일본어, 중국어, 태국어, 베트남어, 인도네시아어, 스페인어, 포르투갈어, 프랑스어, 독일어, 터키어, 아랍어, 힌디어, 러시아어, 벵골어, 우르두어, 네팔어
...
- [x] 완료 시 최종 통합 테스트 후 한국어 커밋
    - [x] 커밋 완료
    - [x] **v1.5 릴리즈**: 벵골어/우르두어/네팔어 추가, RTL 레이아웃 개선, 체험하기 로직 수정, Git Tag/배포 완료
...
- [x] **모바일 접근성 강화**:
    ...
    - [x] **모바일 줌 동작 개선**: 방송국 선택 시 자동 확대 방지 (패닝만 수행) 및 PC 줌 레벨 완화
    - [x] **회전 동작 개선**: 모바일에서 방송국 선택 시 자동 회전 멈추지 않고 유지
...
- [x] **디자인 및 에셋 준비**
    ...
    - [x] `framer-motion` 패키지 확인 및 설치 (이미 있으면 생략)
- [x] **애니메이션 컴포넌트 구현**
    - [x] **Thinking Duck (로딩)**: 위아래 둥실거림, 눈 깜빡임, '...' 말풍선 또는 땀 흘리는 효과
    - [x] **Party Duck (성공)**: 점프, 회전, 팔(날개) 흔들기, 폭죽 이펙트(CSS/Canvas)
    - [x] **Lonely Duck (Empty)**: 고개 숙임, 발 차기, 흑백/채도 감소 효과 고려
    - [x] **Teacher Duck (가이드)**: 특정 방향 가리키기(Props로 방향 제어), 안경 착용 등의 소품 추가
- [x] **페이지/기능 연동**
    - [x] **로딩 적용**: `AiFeedback` 로딩 상태, `Analysis` 로딩 화면에 적용 (DiaryForm 연동 완료), `AuthGate` 초기 로딩 최적화 및 오리 적용
    - [x] **성공 적용**: 일기 저장 완료 모달, 퀴즈 정답 시 오버레이에 적용 (DiaryForm 완료)
    - [x] **Empty 적용**: `DiaryList` 빈 화면, `ArchivePage` 빈 화면에 적용 (DiaryList 완료)
    - [x] **가이드 적용**: 메인 홈 '일기 쓰기' 가이드 또는 튜토리얼 툴팁에 적용 (StartButton 완료)
- [x] **테스트 및 배포**
    - [x] 각 상태별 애니메이션 자연스러움 검증
    - [x] 모바일/PC 반응형 크기 최적화
    - [x] 배포 및 최종 확인
- [x] 완료 시 테스트 통과 확인 후 한국어 커밋
...
## 27. 오리 마스코트 말풍선 스타일 통일 — Gemini
- [x] **말풍선 스타일 고정**:
    - [x] 모바일 다크모드에서 말풍선이 검은색으로 보이는 문제 수정 (흰색 배경/검은 텍스트 고정)
- [x] 완료 시 테스트 통과 확인 후 한국어 커밋

## 28. 모바일 언어 선택 드롭다운 구현 — Gemini
- [x] **모바일 헤더 구현**:
    - [x] `Navigation.tsx`에 모바일(`md:hidden`) 전용 상단 헤더 추가
    - [x] 로고 및 언어 선택 드롭다운 배치
    - [x] `z-index` 조정(50) 및 배경 스타일(Glassmorphism) 적용
- [x] **레이아웃 조정**:
    - [x] `layout.tsx` 컨텐츠 패딩(`py-20`) 추가로 헤더/네비게이션 겹침 방지
- [x] 완료 시 테스트 통과 확인 후 한국어 커밋
- [x] **배포 (v1.6.0)**:
...
## 31. Retention & Onboarding (Version Update)

### 1) 데이터 모델 및 스트릭(Streak) 시스템 코어 (Micro-TDD)
