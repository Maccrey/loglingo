...
- [x] Firestore 스키마(learning_archive, quiz_questions) 리포지토리/타입 정의
- [x] AI 교정 결과 → 자주 틀리는 패턴 자동 저장 로직 구현(일기 폼 AI 결과 → 아카이브 저장 버튼)
- [x] 문제 자동 생성(객관식 4지선다) 및 해설 표시 UI 구현 - Grok AI 기반 자동 생성
- [x] Firebase에 퀴즈 저장/조회 기능 구현, 보기 순서 랜덤화
- [x] 학습 페이지: 필터/검색, 진행 상태 저장, 정답/오답 피드백(간단 퀵퀴즈)
- [x] 퀴즈 정답/오답 시각적 피드백 (초록색/빨간색, 3초 자동 리셋)
- [x] AI 생성 로딩 상태 UI ("AI 생성 중..."), 에러 처리
- [x] 지표 로깅: `quiz_started`, 정답률/재방문 이벤트(placeholder 로그)
- [x] 테스트: 단위/통합, Playwright로 문제 풀기 플로우(작성, 환경변수 `PLAYWRIGHT_START_SERVER=1` 시 실행)
- [x] 완료 시 테스트 통과 확인 후 한국어 커밋
