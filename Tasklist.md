...
- [ ] **레벨 추정 로직**: AI 교정 응답 분석으로 학습어 레벨(CEFR/점수) 추정, 신뢰도·근거 포함
- [ ] **데이터 저장**: `users/{uid}/level` 서브컬렉션에 레벨 기록, `users/{uid}/advice`에 다국어 조언/액션 저장
- [ ] **난이도 반영**: 학습 아카이브/퀴즈 생성 시 최신 레벨을 프롬프트/옵션 난이도에 반영
- [ ] **대시보드 UI**: 학습 페이지 상단 카드/차트로 현재 레벨·추세(7/30일), 우선 조언, 일기/교정/퀴즈 활동량, 평균 대비 표시 (반응형/다국어)
- [ ] **i18n**: 레벨/조언/대시보드 라벨 번역 키 15개 언어 추가, 숫자/날짜 포맷 로캘 적용
- [ ] **지표/이벤트**: `level_recorded`, `advice_clicked`, `dashboard_viewed` 이벤트 로깅
- [ ] **테스트**: 레벨 저장/조언 저장 단위 테스트, 대시보드 렌더/필터/반응형 스냅샷, E2E(레벨 기록→대시보드 반영) 시나리오
- [ ] **추가 고려**:
