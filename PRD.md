...
### 기능 6. 체험 모드 (Trial Mode)
- **제약**: 로그인 없이 사용 가능하나 데이터 저장 불가 (휘발성).
- **1회 제한**: 로컬 스토리지(`loglingo_trial_completed`)를 통해 브라우저당 1회만 체험 가능.
- **UX**:
  - 교정 완료 후 "회원가입하여 저장하세요" 유도 (저장 불가 안내 명시).
  - 체험 완료 유저는 홈 화면 진입 시 "체험하기" 버튼이 "일기 쓰기 시작"으로 변경.
  - 체험 url 재진입 시 홈으로 리다이렉트.
...
- UI 언어와 학습 언어 분리
- **지원 언어 (18개국)**: 한국어, 영어, 일본어, 중국어(간체/번체), 태국어, 베트남어, 인도네시아어, 스페인어, 포르투갈어, 프랑스어, 독일어, 터키어, 아랍어, 힌디어, 러시아어, **벵골어, 우르두어, 네팔어**
...
# Speaking 기능 확장 v1 (Additional PRD)

## 1. 문제 정의 (Speaking)

외국어 학습자는 **말하기 연습을 체계적으로 할 수 있는 환경**이 부족하다.
특히 다음 문제가 존재한다.

* 혼자 말해도 **내 발화가 맞는지 판단 불가**
* 발음·문법·의미 오류를 **구분해서 피드백 받기 어려움**
* 말하기 결과가 **학습 데이터로 축적되지 않음**
...
> “말하기 → 텍스트화 → 언어 분석 → 학습 아카이브화 → 퀴즈화”

* **Google Speech-to-Text**로 학습자 발화를 최대한 왜곡 없이 텍스트화
* **Grok API**로 문법·의미·표현 중심의 학습 피드백 생성
* 결과를 **Learning Archive와 완전히 동일한 구조로 저장**
* 비용 절감을 위해 **AI 호출 최소화 + 재사용 가능 데이터화**
...
* speaking_started → speaking_completed 전환율 ≥ 70%
...
* 말하기에서 발견된:
  ...
  * 자주 틀리는 구조
  * 핵심 단어
* → **learning_archive에 자동 저장**
* origin = `"speaking"`

**중복 체크**

* 동일 rootMeaning + type 조합은 재사용
...
| 방식             | 판단               |
| -------------- | ---------------- |
| 문장마다 즉시 분석     | ❌ (비용·UX·집중도 저하) |
| **발화 완료 후 분석** | **⭕ 채택**         |

**이유**

* STT + Grok 연속 호출 시 비용 폭증
* 말하기 흐름이 끊김
* 학습자는 “말하기”에 집중해야 함
...
### speaking_sessions (신규)

| 필드          | TypeScript | Firestore | 설명     |
| ----------- | ---------- | --------- | ------ |
| id          | string     | string    | 세션 ID  |
| userId      | string     | string    | 사용자    |
| language    | string     | string    | 학습 언어  |
| transcript  | string     | string    | STT 결과 |
| durationSec | number     | number    | 발화 시간  |
| aiAnalyzed  | boolean    | boolean   | 분석 여부  |
| createdAt   | Date       | timestamp | 생성     |
...
| 필드               | TypeScript | Firestore | 설명                   |
| ---------------- | ---------- | --------- | -------------------- |
| id               | string     | string    | 피드백 ID               |
| sessionId        | string     | string    | speaking_sessions FK |
| original         | string     | string    | 원문                   |
| improved         | string     | string    | 개선 문장                |
| grammarNotes     | string[]   | array     | 문법 포인트               |
| rootMeaningGuide | map        | map       | 의미 설명                |
| createdAt        | Date       | timestamp | 생성                   |
...
* **[완료] 일일 무료 말하기 횟수 제한**:
    * **3회/일**로 제한 (비로그인/로그인 유저 공통)
    * 제한 도달 시 **"Daily Limit Modal"** 표시
    * 내일 다시 오기 유도 (재방문 지표 강화)
* **[완료] 맞춤형 AI 프롬프트 생성 (비용 가치 극대화)**:
    * 사용자가 더 많이 말하게 유도하기 위해 Grok 등 외부 LLM에서 사용할 수 있는 **Personalized Prompt** 제공
    * 사용자 데이터(일기, 아카이브, 레벨 등)를 조합하여 "나만을 위한 토픽" 생성
* 프리미엄:
    * 무제한 or 10상향 (추후 구현)
* 동일 transcript 재분석 방지 (hash)
...
| Speaking 완료율           | ≥ 70% |
| 분석 열람률                 | ≥ 80% |
| Speaking → Archive 생성률 | ≥ 60% |
| Speaking 기반 퀴즈 참여율     | ≥ 25% |
| D7 Speaking 재사용        | ≥ 20% |
| **일일 제한 도달율**        | ≥ 15% |
...
# Retention & Onboarding 개선 (버전업데이트)

## 1. 문제 정의 (Retention & Sign-up)
- **가입 전 이탈**: 앱의 핵심 가치(AI 교정, 원어민 발음)를 체험하기 전에 회원가입 장벽으로 이탈
- **작성의 막막함**: 빈 페이지에서 외국어로 일기를 쓰려는 사용자는 무슨 말을 쓸지 몰라 포기 (초기 장벽)
- **동기 부여 부족**: 매일 일기를 쓰는 습관 형성을 위한 즉각적 성취감이나 넛지(Nudge) 부재
- **기계적인 피드백**: AI 교정이 단순 문법 수정에 그쳐 '내 이야기를 들어준다'는 느낌 부족

## 2. 핵심 기능 정의 (Retention v2.0)

### 기능 7. 홈 화면 1문장 게스트 체험 (Aha Moment)
- **입력**: 홈 화면 Hero 섹션에 1~2문장을 바로 적을 수 있는 단문 입력창 제공
- **기능**: 작성 후 버튼 클릭 시 즉시 AI가 교정문과 함께 원어민 발음(TTS)을 들려주며 모달 팝업으로 결과 표시
- **출력**: "이 기록을 저장하고 내일도 스트릭을 이어가려면 가입하세요" 가입 유도
- **i18n 계약**: `enabled` (지원 언어: 18개국, 기본: `ko`). 홈 화면 텍스트 및 가입 유도 모달 문구 다국어 지원 필수.

### 기능 8. 오늘의 질문 (Daily Prompt)
- **입력**: 일기 작성 페이지 진입 시 미리 준비된 가벼운 질문 프롬프트 제공 (예: "최근에 본 영화는 무엇인가요?")
- **기능**: '이 질문 에디터에 넣기' 선택 시 에디터에 내용 템플릿 삽입
- **i18n 계약**: `enabled`. 질문 리스트는 사용자의 UI 언어로 번역되어 제공.

### 기능 9. AI 감성 피드백 (Tikitaka)
- **입력**: 일기 제출 내용
- **기능**: AI 교정 시, 일기 내용에 대한 공감과 응원이 담긴 1~2문장 코멘트를 AI가 '학습 언어'로 추가 생성 (`emotionalComment` 필드)
- **출력**: 교정 결과 상단에 오리 마스코트와 함께 다정한 코멘트 표시 (TTS로 재생 가능)
- **i18n 계약**: `enabled`. 코멘트 자체는 학습 언어로 제공되나, 라벨 및 안내 문구는 UI 언어를 따름.

### 기능 10. 스트릭(Streak) 시스템 & 게이미피케이션
- **상태**: Firestore `users/{userId}`에 `streak` 맵 (`current`, `longest`, `lastEntryDate`)을 기록하여 연속 작성일수 추적
- **출력**: 네비게이션 헤더 및 대시보드에 불꽃 아이콘(🔥)과 연속일수 노출.
- **사이드 이펙트**: 1일(로컬 타임존 자정 기준) 연속 작성 미달 시 `current` 스트릭 초기화
- **i18n 계약**: `enabled`. 스트릭 툴팁 및 달성 안내 문구 지원.

## 3. 데이터 모델 변경 (Firestore)

### 3.1 `users` 내 `streak` 필드
**Path**: `users/{userId}` 도큐먼트 내 `streak` 구조체 추가
| 필드 | 타입 | 설명 |
| --- | --- | --- |
| streak.current | number | 현재 연속 작성일수 |
| streak.longest | number | 역대 최대 연속 작성일수 |
| streak.lastEntryDate | string | 마지막 작성일 (YYYY-MM-DD 형식, 사용자 로컬 타임존 기준 문자열) |
