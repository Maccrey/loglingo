...
- **완전한 다국어 지원**: 모든 포스트는 **18개 언어**로 번역되어야 합니다. (영어/한국어 외 다른 언어에 영어 콘텐츠를 복사해 넣는 행위 금지)
- **콘텐츠 품질**: 각 언어별 콘텐츠는 **약 1600자** 분량의 풍부한 내용을 담아야 합니다.
...
2.  **번역 (Translation)**: AI 또는 번역 도구를 사용하여 나머지 17개 언어로 **완벽하게 번역**합니다.
    - *주의: 단순히 영어를 복사해 넣으면 안 됩니다. 각 언어의 문맥에 맞는 번역이 필요합니다.*
3.  **코드 구현**: `scripts/blog-data-partX.ts` 파일에 상수와 객체를 추가합니다.
4.  **DB 시딩 (Seeding)**: 다음 명령어를 실행하여 Firestore에 데이터를 반영합니다.
    ```bash
...
- [ ] `id`가 중복되지 않는가?
- [ ] `title`, `excerpt`, `content`에 18개 언어 키가 빠짐없이 모두 있는가?
